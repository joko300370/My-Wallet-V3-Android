package piuk.blockchain.android.ui.swapold.customviews
...
sealed class FloatKeyboardIntent {
    ...
    object Period : FloatKeyboardIntent()
    object Backspace : FloatKeyboardIntent()
    object Clear : FloatKeyboardIntent()
...
class FloatKeyboardDialog(intents: Observable<FloatKeyboardIntent>) {
    val states: Observable<FloatEntryViewState> =
        intents.scan(initialState) { previous, intent ->
            when (intent) {
                ...
                is FloatKeyboardIntent.SetValue -> constructViewStateFromValue(
                    previous,
                    initialState,
                    intent
                )
    ...
    private fun mapKeyPress(previous: FloatEntryViewState, key: Int): FloatEntryViewState {
        return when {
            ...
            if (previous.maximums.maxIntLength in 1 until integerDigits) {
                previous.shake()
            } else if (previous.maximums.maxDigits in 1 until allDigits) {
                previous.shake()
    ...
    private fun mapPeriodPress(previous: FloatEntryViewState): FloatEntryViewState {
        return when {
            ...
            previous.decimalCursor > 1 -> previous.copy(shake = true)
                ...
                userDecimal = previous.userDecimal.setScale(previous.maxDecimal),
                decimalCursor = 1,
                shake = false,
                previous = previous
            )
    ...
    private fun constructViewStateFromValue(
        ...
        val intents = listOf(FloatKeyboardIntent.SetMaximums(previous.maximums)) + numberToIntents(intent)
    ...
    private fun numberToIntents(intent: FloatKeyboardIntent.SetValue): List<FloatKeyboardIntent> {
        ...
        val onePlusFraction =
            (BigDecimal.ONE + intent.value - integer.toBigDecimal()).movePointRight(intent.maxDp)
                .toBigInteger()

        val integerIntents =
            listOf(FloatKeyboardIntent.SetMaxDp(intent.maxDp)) + intToIntents(integer, false)
...
data class FloatEntryViewState(
    ...
    fun shake() = copy(shake = true)
    fun setMaximum(intent: FloatKeyboardIntent.SetMaximums) =
