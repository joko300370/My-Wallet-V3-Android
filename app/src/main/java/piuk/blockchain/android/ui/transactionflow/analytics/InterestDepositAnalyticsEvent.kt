...
sealed class InterestDepositAnalyticsEvent(
    ...
    ) : InterestDepositAnalyticsEvent("earn_deposit_confirm_click", params = mapOf("asset" to asset.networkTicker))

    data class EnterAmountCtaClick(
        val asset: CryptoCurrency
    ) : InterestDepositAnalyticsEvent("earn_amount_screen_confirm", params = mapOf("asset" to asset.networkTicker))

    data class TransactionSuccess(
        val asset: CryptoCurrency
    ) : InterestDepositAnalyticsEvent("earn_checkout_success", params = mapOf("asset" to asset.networkTicker))

    data class TransactionFailed(
        val asset: CryptoCurrency
    ) : InterestDepositAnalyticsEvent("earn_checkout_error", params = mapOf("asset" to asset.networkTicker))
}
