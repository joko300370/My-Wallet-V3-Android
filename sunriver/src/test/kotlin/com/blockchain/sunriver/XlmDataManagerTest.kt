...
class XlmDataManagerSendTransactionTest {
    ...
    fun `can send`() {
        val horizonProxy: HorizonProxy = mock {
            on {
                sendTransaction(
                    source = keyPairEq(KeyPair.fromSecretSeed("SCIB3NRLJR6BPQRF3WCSPBICSZIXNLGHKWDZZ32OA6TFOJJKWGNHOHIA")),
                    destination = keyPairEq(KeyPair.fromAccountId("GDKDDBJNREDV4ITL65Z3PNKAGWYJQL7FZJSV4P2UWGLRXI6AWT36UED3")),
                    amount = eq("199.456000".toBigDecimal())
                )
